// LangChain.js 代理用于安卓自动化（Electron版本）
// 使用IPC与主进程通信以执行ADB操作
...
/** 使用Electron IPC构建LangChain兼容的工具 */
function buildLangChainTools(onCapture?: (base64: string) => void) {
  const captureScreenshot: DynamicStructuredTool = tool(
    async () => {
      try {
        ...
        // 返回 base64 图片数据和分辨率，让大模型能够"看到"屏幕并知道坐标范围
        return `SCREENSHOT:${capture.width}x${capture.height}:${capture.base64}`;
      } catch (error) {
        return `截屏失败: ${error}`;
      }
    },
  ...
  const tapScreen: DynamicStructuredTool = tool(
    async ({ x, y }: { x: number; y: number }) => {
      ...
      await window.electronAPI.tap(pixel.x, pixel.y);
      // 等待点击后UI响应
  ...
  const swipeScreen: DynamicStructuredTool = tool(
    async ({ x1, y1, x2, y2, duration }: { x1: number; y1: number; x2: number; y2: number; duration?: number }) => {
      ...
      await window.electronAPI.swipe(start.x, start.y, end.x, end.y, duration ?? 300);
      // 等待滑动后UI响应
      await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  const shellCommand: DynamicStructuredTool = tool(
    async ({ command }: { command: string }) => {
      const output = await window.electronAPI.shell(command);
      // 等待命令完成
  ...
  const longPress: DynamicStructuredTool = tool(
    async ({ x, y, duration }: { x: number; y: number; duration?: number }) => {
      // 归一化坐标转像素坐标
      const pixel = normalizedToPixel(x, y);
      await window.electronAPI.longPress(pixel.x, pixel.y, duration ?? 1000);
      // 等待长按操作完成
      await new Promise((resolve) => setTimeout(resolve, 500));
...
/**
 * 使用LangChain.js工具调用的安卓自动化代理（Electron版本）。
 */
export class AndroidAgent {
  ...
  async run(task: string): Promise<string> {
    ...
    try {
      ...
      const tools = buildLangChainTools(this.onScreenCapture);
...
请根据上述技能指导继续执行任务。`;
  ...
  updateConfig(config: Partial<AgentConfig>) {
    ...
    this.visionService.updateConfig({
...
// 为aiStore设置代理工厂
import { setAgentFactory } from '../../store/aiStore';

setAgentFactory((config, callbacks) => new AndroidAgent(config, callbacks));
